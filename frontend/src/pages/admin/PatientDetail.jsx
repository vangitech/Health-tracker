...
import { ArrowLeft, Phone, Mail, Calendar, Loader2, Activity, AlertTriangle, CheckCircle } from 'lucide-react';
...
export default function PatientDetail() {
  ...
  const statCards = stats
    ? [
        ...
        {
          label: 'Avg Glucose',
          value: stats.averageGlucose != null ? stats.averageGlucose.toFixed(1) : '—',
          icon: Activity,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => navigate('/iaccess/patients')} className="shrink-0">
          ...
          {appointments.length === 0 ? (
            ...
            <div className="space-y-3">
              {appointments.map((apt) => (
                <div key={apt._id || apt.id} className="flex items-start gap-3 p-3 rounded-lg border border-zinc-800">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {apt.notes && <p className="text-sm text-zinc-400 mt-1">{apt.notes}</p>}
                  </div>
                </div>
              ))}
            </div>
