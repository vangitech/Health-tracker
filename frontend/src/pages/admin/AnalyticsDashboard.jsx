  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-1">Welcome back, {admin?.firstName || admin?.email || 'Admin'}</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map(({ key, label, icon: Icon, color, bg }) => (
          <Card
            key={key}
            className="transition-all duration-200 hover:scale-[1.02] hover:bg-zinc-800/80 cursor-default"
          >
            ...
            <CardContent>
              <p className="text-2xl font-bold text-zinc-100">{data?.[key] ?? 0}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-medium text-zinc-400">Weekly Registrations</CardTitle>
        </CardHeader>
        <CardContent>
