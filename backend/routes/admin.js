...
router.post('/register', authenticateAdmin, requireSuperAdmin, async (req, res) => {
  try {
    ...
    if (password.length < 8 || !/\d/.test(password) || !/[^a-zA-Z0-9]/.test(password)) {
      return res
        .status(400)
        .json({ message: 'Password must be at least 8 characters with at least one number and one special character' });
    }
...
router.get('/analytics', authenticateAdmin, async (req, res) => {
  try {
    ...
    const totalAdmins = await User.countDocuments({
      role: { $in: ['admin', 'superadmin', 'doctor', 'recordofficer', 'nurse'] },
    });
    ...
    const result = await Promise.all(
      patients.map(async (patient) => {
        const [latestEntry, lastAppointment, nextAppointment] = await Promise.all([
          ...
          Appointment.findOne({ userId: patient._id, status: { $ne: 'cancelled' } })
  ...
  try {
    ...
    const latestEntries = await BloodSugarEntry.aggregate([
      ...
      { $sort: { date: -1, time: -1 } },
      {
        $group: {
          _id: '$userId',
          glucoseValue: { $first: '$glucoseValue' },
          date: { $first: '$date' },
          time: { $first: '$time' },
        },
      },
...
router.get('/patients/:id', authenticateAdmin, async (req, res) => {
  try {
    const patient = await User.findOne({ _id: req.params.id, ...patientFilter() })
      .select('-password')
      .lean();
...
router.put('/profile/change-password', authenticateAdmin, async (req, res) => {
  try {
    ...
    if (!/\d/.test(newPassword)) return res.status(400).json({ message: 'Password must include at least one number' });
